...
import { type User, type Tenant, type Branch, type LoginResponse, Role } from "@/types/auth";
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [tenant, setTenant] = useState<Tenant | null>(() => loadJson<Tenant>(TENANT_KEY));
  const [branches, setBranches] = useState<Branch[]>(() => loadJson<Branch[]>(BRANCHES_KEY) ?? []);
  ...
  const login = useCallback((response: LoginResponse) => {
    const { accessToken, refreshToken, user: u, tenant: t, branches: b } = response;
