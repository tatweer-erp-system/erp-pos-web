  ...
  const mergedItemCount = useMemo(() => {
    ...
    for (const { order } of selectedOrders) {
      for (const item of order.cartItems) {
        map.set(
          item.product.id,
          (map.get(item.product.id) ?? 0) + item.quantity
        );
      }
    }
